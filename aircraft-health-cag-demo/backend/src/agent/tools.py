...
CDF graph traversal tools for the ReAct agent, using:
  ...
  - httpx for custom fleet resource routes (symptoms, policies, etc.)
...
def get_aircraft_symptoms(aircraft_id: str) -> dict[str, Any]:
    """
    Retrieve observed symptoms for a specific aircraft.
    Combines custom SymptomNode list with EXHIBITED relationship lookups.
    """
    log_traversal(f"Symptoms:{aircraft_id}")
    data = _cdf_post("symptoms/list", {"filter": {"aircraft_id": aircraft_id}})
    symptoms = data.get("items", [])
...
def search_fleet_for_similar_events(description: str) -> dict[str, Any]:
    ...
    Searches the following free-text fields:
      ...
      - SymptomNodes: description, observation, title fields
    ...
    # Search symptom nodes (all text fields)
    sym_data = _cdf_post("symptoms/list", {})
    for sym in sym_data.get("items", []):
        score = (
            _score(sym.get("description", "")) +
            _score(sym.get("observation", "")) +
            _score(sym.get("title", ""))
        )
        if score > 0:
            matches.append({
                "score": score,
                "type": "SymptomNode",
                "externalId": sym.get("externalId", ""),
                "tail": sym.get("aircraft_id", ""),
                "description": sym.get("description", "")[:200],
                "observation": sym.get("observation", "")[:200],
                "title": sym.get("title", ""),
...
def symptom_fleet_deep_dive(aircraft_id: str, symptoms: dict[str, Any]) -> Optional[dict[str, Any]]:
    """
    When an aircraft has SymptomNodes, fetch cross-asset context: same engine model
...
TOOL_DEFINITIONS: list[dict[str, Any]] = [
    ...
    },
    {
        ...
        "description": (
            ...
            "HAS_COMPONENT, IS_TYPE, GOVERNED_BY, HAS_POLICY, EXHIBITED, LINKED_TO, PERFORMED_ON, IDENTIFIED_ON."
        ),
        "input_schema": {
            "type": "object",
            "properties": {
    ...
    },
    {
        ...
        "description": "Get observed SymptomNodes for a specific aircraft.",
        "input_schema": {
            "type": "object",
            "properties": {
